...
2. «Переносимость» эффектов. Историю делим на «похожих на аудиторию» и «непохожих»
   ...
   неопределённости добавлять (TRANSFER_SD агента). Для сравнения — тот же расчёт на случайном
...
def main():
    ...
    result = {"distributions": comp,
              ...
              "recommendation": {
                  ...
                  "TRANSFER_SD_in_q_units": round(tr_shift["resid_sd"], 3),
    ...
    print(f"Доверие к истории по данным — не выше β≈{upper:.2f}; реальная аудитория отличается сильнее → "
          f"PRIOR_SHRINK 0.7–0.8, TRANSFER_SD ≈ {tr_shift['resid_sd']:.3f} в единицах q")
