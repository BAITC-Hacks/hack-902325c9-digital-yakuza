...
import { campaignId, estimateLabel } from '../utils/explanation';
import { RiskInfo } from './Explanation';
...
export function Campaigns({ run }: { run: AgentRun | null }) {
  ...
  return <>
    <p className="result-version">seed {run.seed} · агент {typeof (run.agent_sha256 ?? run.metrics?.agent_sha256) === 'string'
      ? String(run.agent_sha256 ?? run.metrics?.agent_sha256).slice(0, 8) : 'версия недоступна'}</p>
    ...
    {run.campaigns.length ? <DataTable label="Финальные кампании"
      headers={['Кампания', 'Целевой тариф', 'Канал', 'Сегмент', 'Охват', 'Затраты, у.е.', { label: 'Прирост дохода, у.е.', help: 'Ожидаемый прирост дохода до вычета затрат на коммуникацию.' }, { label: 'Ограничения охвата', help: 'Показывает, какие лимиты сократили охват кампании: размер одной кампании, общее число контактов или бюджет.' }, 'Подтверждено пилотами']}
      rows={run.campaigns.map((campaign, index) => [
        <div className="campaign-cell"><strong>{campaignId(run, campaign, index)} · {campaignLabel(campaign.campaign_name)}</strong>
          {typeof campaign.estimate_source === 'string' && <small>Источник оценки: {estimateLabel(campaign.estimate_source)}</small>}
          {!!campaign.risk_info && typeof campaign.risk_info === 'object' && <RiskInfo value={campaign.risk_info as Record<string, unknown>} />}
        </div>, displayValue(campaign.target_tariff), <span className="tag">{displayValue(campaign.channel)}</span>,
        ...
        run.explanation?.campaign_pilot_links?.[campaignId(run, campaign, index)]?.join(', ') || '—',
